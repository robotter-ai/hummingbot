...
class GatewaySwap(GatewayBase):
    ...
    async def _create_order(
        self, trade_type: TradeType, order_id: str, trading_pair: str, amount: Decimal, price: Decimal
    ):
        """
        ...
        try:
            order_result: Dict[str, Any] = await self._get_gateway_instance().execute_swap(
                self.network, self.connector_name, self.address, base, quote, trade_type, amount
            )
