...
class GatewayHttpClient:
        ...
        try:
            ...
            if not fail_silently:
                if self.is_timeout_error(e):
                    ...
                    self.logger().network(
                        e, exc_info=True, app_warning_msg=f"Call to {url} failed. See logs for more details."
                    )
                raise e
    ...
    async def get_gateway_status(self, fail_silently: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger().network("Error fetching gateway status info", exc_info=True, app_warning_msg=str(e))
    ...
    async def get_tokens(self, chain: str, network: str, fail_silently: bool = True) -> Dict[str, Any]:
        return await self.api_request(
            "get", "chain/tokens", {"chain": chain, "network": network}, fail_silently=fail_silently
        )

    async def get_algorand_assets(self, network: str, fail_silently: bool = True) -> Dict[str, Any]:
        return await self.get_tokens(**{"chain": "algorand", "network": network, "fail_silently": fail_silently})
    ...
    async def get_allowances(
        ...
        return await self.api_request(
            "post",
            "chain/allowances",
            {"chain": chain, "network": network, "address": address, "tokenSymbols": token_symbols, "spender": spender},
            fail_silently=fail_silently,
        )
    ...
    async def cancel_evm_transaction(self, chain: str, network: str, address: str, nonce: int) -> Dict[str, Any]:
        return await self.api_request(
            "post", "chain/cancel", {"chain": chain, "network": network, "address": address, "nonce": nonce}
        )
    ...
    async def get_perp_market_status(
        self, chain: str, network: str, connector: str, base_asset: str, quote_asset: str, fail_silently: bool = False
    ) -> Dict[str, Any]:
        return await self.api_request(
            "post",
    ...
    async def amm_lp_position(
        self, chain: str, network: str, connector: str, token_id: int, address: Optional[str] = ""
    ) -> Dict[str, Any]:
        request_payload: Dict[str, Any] = {
            "chain": chain,
            "network": network,
    ...
    async def get_clob_orderbook_snapshot(
        ...
        request_payload = {"market": trading_pair, "connector": connector, "chain": chain, "network": network}
    ...
    async def clob_perp_funding_payments(
        self, address: str, chain: str, connector: str, network: str, trading_pair: str, **kwargs
    ):
        request_payload = {
            "chain": chain,
            "network": network,
    ...
    async def clob_perp_get_orders(
        ...
        request = {"chain": chain, "network": network, "connector": connector, "market": market}
    ...
    async def clob_perp_get_order_trades(
        ...
        request = {"chain": chain, "network": network, "connector": connector, "address": address, "orderId": order_id}
    ...
    async def clob_perp_last_trade_price(
        ...
        request_payload = {"chain": chain, "network": network, "connector": connector, "market": trading_pair}
    ...
    async def clob_perp_cancel_order(
        self, chain: str, network: str, connector: str, address: str, trading_pair: str, exchange_order_id: str
    ) -> Dict[str, Any]:
        request_payload = {
            "chain": chain,
            "network": network,
