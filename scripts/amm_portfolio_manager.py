...
class AMMRobustPositionManager(ScriptStrategyBase):
    ...
    Permanent (static) database data is built once and then dynamic fields
    are updated periodically by asyncio tasks. The arbitrage strategy executes
    only after the dynamic database has been updated at least once.
    """
    ...
    _last_arbitrage_check_time: Decimal = Decimal("0")
    ...
    def __init__(self, connectors: Dict[str, ConnectorBase]):
        """
        Initialize the strategy: load configuration, build the permanent database,
        and schedule dynamic updates.
    ...
    def _initialize(self, strategy_configuration: Optional[AMMRobustPositionManagerConfiguration] = None):
        ...
        self._maximum_slippage_percentage = Decimal(globals_conf.get("maximum_slippage_percentage", "0"))
        self._minimum_profitability_percentage = Decimal(globals_conf.get("minimum_profitability_percentage", "0"))
        self._arbitrage_check_interval_seconds = Decimal(globals_conf.get("arbitrage_check_interval_seconds", "0"))
        self._minimum_trade_amount = Decimal(globals_conf.get("minimum_trade_amount", "0"))
        self._time_delay_between_arbitrages = Decimal(globals_conf.get("time_delay_between_arbitrages", "0"))
        self._transaction_confirmation_delay = Decimal(globals_conf.get("transaction_confirmation_delay", "0"))
        ...
        self._is_already_running = bool(globals_conf.get("is_already_running", False))
        ...
        if self._transaction_confirmation_delay > DECIMAL_ZERO:
    ...
    def _log_initialization(self):
        ...
        self.logger().info(f"Async data updates enabled: {self._use_async_data_updates}")

    def _start_data_update_task(self):
        """Schedule the asynchronous dynamic update loop."""
    ...
    async def _run_data_update_loop(self):
        """
        Asynchronous loop to update dynamic data:
          - Gateway status
    ...
    def on_tick(self):
        """Called on each tick to schedule execution of the strategy."""
        if not self._is_already_running:
            safe_ensure_future(self._async_on_tick())

    async def _async_on_tick(self):
        """
        Main tick method. The strategy executes only if the dynamic database has been updated
        at least once and the gateway is available.
        """
        try:
            ...
            if not self._initial_db_update_done:
                self.logger().info("Dynamic database not fully updated yet; skipping arbitrage check.")
                return
    ...
    async def _check_gateway_status(self):
        """
        Check the availability of the gateway by sending a ping.
        Set _gateway_is_ready to True if the gateway responds, else False.
        """
        try:
            ...
            if ping_result:
                ...
                self.logger().warning("Gateway ping returned no response.")
        except Exception as e:
            self._gateway_is_ready = False
    ...
    async def _run_arbitrage_strategy(self):
        ...
        Execute the arbitrage strategy:
         - Discover promising token pairs and arbitrage opportunities.
         - Validate each opportunity.
         - Execute arbitrage trades for opportunities that pass validation.
        """
        self.logger().info("Running arbitrage strategy: discovering opportunities...")
        ...
        for opp in opportunities:
            ...
            if valid:
                ...
                if self._time_delay_between_arbitrages > DECIMAL_ZERO:
                        ...
                        for token_sym in self._configuration.get("tokens", []):
                            if token_sym not in hydration["tokens"]:
                                hydration["tokens"][token_sym] = {
                                    ...
                                    "address": None,  # To be updated from gateway
                                    "chain": chain,
                                    "network": network,
                                    "connector": connector,
                                    "symbol": token_sym,
    ...
    async def _update_database(self):
        """
        Update dynamic parts of the database:
         - Pool statistics (APR, TVL, volume, token prices)
         - Wallet balances and pool positions
    ...
    def _find_most_promising_token_pairs(self) -> List[Dict[str, Any]]:
        """
        Identify token pairs with high arbitrage potential based on current pool data.

        Returns:
            List of token pair dictionaries including metrics like price variance.
        """
        ...
        for i in range(len(tokens)):
            for j in range(i + 1, len(tokens)):
                ...
                if len(pools) >= 2:
                    total_volume = sum(Decimal(str(p.get("volume", {}).get("24h", 0))) for p in pools)
                    total_liquidity = sum(Decimal(str(p.get("total_value_locked", 0))) for p in pools)
                    prices = []
                    for pool in pools:
    ...
    def _get_cached_token_price_in_pool(
        ...
        """
        Retrieve the cached price for a token pair from pool data.
    ...
    async def _calculate_price_difference_percentage(
        ...
        """
        Calculate the percentage price difference between two pools.
    ...
    async def _find_arbitrage_opportunities(self, promising_pairs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Find arbitrage opportunities based on promising token pairs.

        Args:
            promising_pairs: List of token pair dictionaries.
        Returns:
            ...
            for i in range(len(pools)):
                for j in range(i + 1, len(pools)):
                    ...
                    if abs(diff) > min_profitability:
                        ...
                        self.logger().info(
                            f"Found arbitrage opportunity: {base_token}/{quote_token} diff {abs(diff):.2f}% between "
                            f"{buy_pool.get('address')} and {sell_pool.get('address')}"
                        )
    ...
    async def _validate_opportunity(self, opportunity: Dict[str, Any]) -> bool:
        """
        Validate an arbitrage opportunity by checking available balances and simulating quotes.

        Args:
            opportunity: Arbitrage opportunity dictionary.
        Returns:
            True if opportunity is valid and expected profit meets threshold; otherwise, False.
        """
        ...
        if available_balance < self._minimum_trade_amount:
            self.logger().info(f"Insufficient {base_token} balance: {available_balance}")
            return False
        ...
        if not trade_amount or trade_amount <= DECIMAL_ZERO:
            self.logger().info("Optimal trade amount invalid")
            return False
        ...
        try:
            ...
            if profit_percentage < self._minimum_profitability_percentage:
                self.logger().info(
                    f"Opportunity not profitable after slippage: {profit_percentage:.2f}% < {self._minimum_profitability_percentage}%"
                )
                return False
    ...
    async def _calculate_optimal_trade_amount(self, opportunity: Dict[str, Any], max_available: Decimal) -> Decimal:
        """
        Determine the optimal trade amount by simulating profits at different sizes.
    ...
    async def _simulate_arbitrage_profit(self, opportunity: Dict[str, Any], amount: Decimal) -> Decimal:
        """
        Simulate expected profit percentage for a given trade amount.
    ...
    async def _execute_arbitrage(self, opportunity: Dict[str, Any]) -> bool:
        """
        Execute an arbitrage trade by performing sequential swaps.
    ...
    async def _get_total_token_balance_from_all_wallets(self, token: str) -> Decimal:
        """
        Sum the free balance of a token across all wallets in the database.
    ...
    async def _get_wallet_addresses_for_pool(self, pool: Dict[str, Any]) -> List[str]:
        """
        Retrieve wallet addresses associated with a given pool.
    ...
    async def _get_quote_swap(
        ...
        """
        Get a swap quote for a given pool and token pair using the gateway.
    ...
    async def _post_execute_swap(
        ...
        """
        Execute a swap transaction via the gateway.
    ...
    async def _wait_for_transaction_confirmation(
        ...
        """
        Wait for a transaction to be confirmed via polling the gateway.
    ...
    async def _gateway_ping_gateway(self):
        """Ping the gateway server to verify connectivity."""
    ...
    async def _gateway_get_tokens(
        ...
        """Retrieve token information from the gateway."""
    ...
    async def _gateway_get_balances(
        ...
        """Retrieve token balances for a wallet address from the gateway."""
    ...
    async def _gateway_quote_swap(
        ...
        """Request a swap quote from the gateway."""
    ...
    async def _gateway_poll_transaction(self, chain: str, network: str, tx_hash: str):
        """Poll the transaction status from the gateway."""
