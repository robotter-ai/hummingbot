...
class AMMRobustPositionManager(ScriptStrategyBase):
            ...
            for token_b in tokens:
                ...
                for i, pool_1 in enumerate(relevant_pools):
                    for j, pool_2 in enumerate(relevant_pools):
                        ...
                        price_difference = await self._calculate_price_difference(
                            token_a, token_b, pool_1, pool_2
                        )
    ...
    async def _execute_arbitrage(self, opportunity: Dict[str, Any]) -> bool:
        ...
        if actual_profit > 0:
            self.logger().info(f"Arbitrage trade successful! Profit: {actual_profit} {token_a} ({actual_profit_pct:.2f}%)")
        else:
            self.logger().warning(f"Arbitrage trade completed with loss: {actual_profit} {token_a} ({actual_profit_pct:.2f}%)")
        ...
        for connection in all_gateway_connections:
            ...
            if wallet_token_balances and "balances" in wallet_token_balances:
                for token_symbol, balance in wallet_token_balances["balances"].items():
                    ...
                    database["connections"][chain][network][connector]["wallets"][wallet_address]["tokens"][token_symbol]["balances"] = {
                        "free": balance,
                        "total": balance
                    }
        ...
        for connection in all_gateway_connections:
            ...
            for pool in pools_info:
                ...
                database["connections"][chain][network][connector]["pools"][pool_address] = {
                    ...
                    "volume": {
                        "24h": detailed_pool_info.get("volume24h")
                    }
                }
                
                # Update token information in the pool
                for token_symbol in detailed_pool_info.get("tokens", {}):
                    token_info = detailed_pool_info["tokens"].get(token_symbol, {})
                    
                    database["connections"][chain][network][connector]["pools"][pool_address]["tokens"][token_symbol] = {
                        "price": token_info.get("price")
                    }

    async def _calculate_price_difference(self, token_a: str, token_b: str, pool_1: Dict[str, Any], pool_2: Dict[str, Any]) -> float:
        """
    ...
    def _find_pools_with_token_pair(self, pools: List[Dict[str, Any]], token_a: str, token_b: str) -> List[Dict[str, Any]]:
        """Find pools that contain both tokens"""
        matching_pools = []
    ...
    def _get_wallet_address_for_pool(self, pool: Dict[str, Any]) -> Optional[str]:
        ...
        for connection in all_gateway_connections:
            if (connection.get("chain") == chain and 
                connection.get("network") == network and 
                connection.get("connector") == connector):
